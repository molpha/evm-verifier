...
library LibMuSig2KeyAgg {
    ...
    function _lexCmp(
        LibSecp256k1.Point memory a,
        LibSecp256k1.Point memory b
    ...
    function _sortPubkeys(
        LibSecp256k1.Point[] memory pts
    ) private pure returns (LibSecp256k1.Point[] memory sorted) {
    ...
    function _hashPubkeyList(
        LibSecp256k1.Point[] memory sorted
    ...
    function _coeffModQ(
        bytes32 L,
        LibSecp256k1.Point memory x
    ) private pure returns (uint256) {
        uint256 a = uint256(
            keccak256(
                abi.encodePacked(
                    bytes("MOLPHA_MUSIG2_COEFF_V1"),
                    bytes32(L),
                    bytes32(x.x),
                    bytes32(x.y)
                )
    ...
    function aggregateKeys(
        LibSecp256k1.Point[] memory participantPubkeys
    ) internal pure returns (LibSecp256k1.Point memory) {
    ...
    function aggregateRegistryKeys(
        LibSecp256k1.Point[] memory registryWithPlaceholder
    ) internal pure returns (LibSecp256k1.Point memory) {
